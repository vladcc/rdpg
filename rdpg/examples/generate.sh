...
readonly G_SRC="infix_calc_grammar.txt"
G_VERBOSE=""
...
function rdpg { awk -f ../rdpg_ir.awk -f ../rdpg.awk $G_SRC; }
function rdpg_opt { awk -f ../rdpg_ir.awk -f ../rdpg-opt.awk "-vOlvl=$1"; }

function rdpg_to_c { awk -f ../rdpg_ir.awk -f ../rdpg-to-c.awk; }
function add_c_src
{
awk 'FNR == 1 {
	print "#include \"lex.c\""
...
function rdpg_to_awk { awk -f ../rdpg_ir.awk -f ../rdpg-to-awk.awk; }
